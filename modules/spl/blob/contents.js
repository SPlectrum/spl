...
exports.default = function spl_blob_contents ( input ) {
    ...
    for ( var i=0; i<sources.length; i++ ) {
        ...
        spl.wsSet ( input, `spl/blob.${spl.fURI ( sources[i].repo, sources[i].dir )}`, contents );
        if( sources[i].reference ) 
            for(var j=0; j<sources[i].reference.length; j++) 
                spl.wsSet( input, sources[i].reference[j], `spl/blob.${spl.fURI ( sources[i].repo, sources[i].dir )}` );
